    ...
    >
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {t('deleteDescPrefix')} <span className='bg-foreground text-primary-foreground rounded px-1'>{tableDelete?.number}</span> {t('deleteDescSuffix')}
          </AlertDialogDescription>
        </AlertDialogHeader>
        <AlertDialogFooter>
          <AlertDialogCancel>{t('cancel')}</AlertDialogCancel>
          <AlertDialogAction onClick={deleteTable}>{t('confirmDelete')}</AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
  )
...
export default function TableTable() {
  ...
  const columns: ColumnDef<TableItem>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => <div>{tTableStatus(row.getValue<string>('status'))}</div>
    },
    {
      accessorKey: 'token',
      header: t('columnQrCode'),
      cell: ({ row }) => <div>{<ExportQRCodeTable token={row.getValue('token')} tableNumber={row.getValue('number')} />}</div>
    },
    {
      id: 'actions',
      enableHiding: false,
      cell: function Actions({ row }) {
        ...
        const tAction = useTranslations('ManageTables')
        const openEditTable = () => {
        ...
        return (
          <DropdownMenu modal={false}>
            ...
            <DropdownMenuContent align='end'>
              <DropdownMenuLabel>{tAction('actions')}</DropdownMenuLabel>
              <DropdownMenuSeparator />
              <DropdownMenuItem onClick={openEditTable}>{tAction('edit')}</DropdownMenuItem>
              <DropdownMenuItem onClick={openDeleteTable}>{tAction('delete')}</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        )
      }
